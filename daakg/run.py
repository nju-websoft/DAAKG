    ...
    def train_and_eval(self):
        ...
        if self.save != "":
            ...
            if 'mtranse_align' in self.args.decoder:
                np.save(self.save + "/%s_mapping.npy" % (time_str), knowledge_decoder[-1].func.weight.cpu().detach().numpy())
...
if __name__ == '__main__':
    ...
    if args.encoder is None:
        args.encoder = ""
    d = AlignmentData(data_dir=args.data_dir, rate=args.rate, share=args.share, swap=args.swap, val=args.val, with_r=args.encoder.lower()=="naea", al=args.al)
    logger.info(d)
