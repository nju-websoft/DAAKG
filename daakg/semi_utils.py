...
def bootstrapping(
    ref_sim_mat, ref_ent1, ref_ent2, labeled_alignment, th, top_k, is_edit=False
):
...
def check_alignment(aligned_pairs, all_n, context="", is_cal=True):
    ...
    for x, y in aligned_pairs:
        if x == y:
            num += 1
    print(
        "{}, right alignment: {}/{}={:.3f}".format(
            context, num, len(aligned_pairs), num / len(aligned_pairs)
        )
    )
    if is_cal:
...
def generate_alignment(sim_mat, sim_th, k, all_n):
    ...
    if neighbors is not None:
        ...
        if len(potential_aligned_pairs) == 0:
            return None, None
        check_alignment(
            potential_aligned_pairs, all_n, context="after sim and neighbours filtered"
        )
    del neighbors
...
def update_labeled_alignment(labeled_alignment, curr_labeled_alignment, sim_mat, all_n):
    ...
    check_alignment(
        labeled_alignment, all_n, context="after editing labeled alignment (<-)"
    )
...
def update_labeled_alignment_label(labeled_alignment, sim_mat, all_n):
    ...
    check_alignment(
        updated_alignment, all_n, context="after editing labeled alignment (->)"
    )
