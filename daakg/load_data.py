# Based on external source codes: https://github.com/THU-KEG/EAkit/blob/master/load_data.py
...
class AlignmentData:
    ...
    def __init__(self, data_dir="./data/daakg/D_W_100K_V1", rate=0.3, share=False, swap=False, val=0.0, with_r=False, al=None):
        t_ = time.time()
        ...
        if al is not None and type(al) == str and len(al) > 0:
            rate = int(rate)
            self.ill_train_idx = np.array(self.load_triples(f"{data_dir}/split/{al}/{rate}_train", file_num=1))
            self.ill_val_idx = np.array([])
            self.ill_test_idx = np.array(self.load_triples(f"{data_dir}/split/{al}/{rate}_test", file_num=1))
            self.ill_idx = np.concatenate([self.ill_train_idx, self.ill_test_idx])
            # self.ill_idx = self.load_triples(data_dir + "/ill_ent_ids", file_num=1)
        else:
